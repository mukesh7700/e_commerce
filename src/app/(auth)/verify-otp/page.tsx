"use client";
import { Button, Card, CardContent, toast } from "@jamsr-ui/react";
...
const Page = () => {
  ...
  const handleOtpChange = (
    ...
    if (index < 5 && value) {
      otpRefs.current[index + 1]?.focus(); // ✅ Safe access
    }
  };
  ...
  if (/^\d{6}$/.test(pastedData)) {
    ...
    newOtp.forEach((digit, i) => {
      if (otpRefs.current[i]) {
        otpRefs.current[i]!.value = digit; // Set input value manually
      }
    });
    otpRefs.current[5]?.focus(); // Move focus to last input
  }
};
